...
ar_char_replacements = {
    ...
    u'\u0649': u'\u064a',
    u'\u06cc': u'\u064a',
    u'\u0626': u'\u064a' #todo: must ignore for abjad
    }
...
abjad_values = {
    ...
    u'\u0636': 800, u'\u0638': 900, u'\u063a': 1000, u'\u0621': 1, u'\u0626': 1
    }
...
def normalize(text):
    ...
    text = u''.join([ar_char_replacements.get(c, c) for c in text])
    return text
...
def abjad_value(text):
    #is tah marbuta equivalent to hah for abjad?
    #is hamza value 1 regardless of its seat?
    return sum([abjad_values[c] for c in normalize(text)])
